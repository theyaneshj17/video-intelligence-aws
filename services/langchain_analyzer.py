...
# LangChain-based intelligence analysis with Claude Haiku
# Separation of concerns: Only handles AI analysis and processing
...
from transcript_extractor import TranscriptSegment, VideoMetadata
...
class BabblIntelligenceAnalyzer:
    """
    LangChain-based video intelligence analyzer using Claude Haiku
    Optimized for financial and business intelligence extraction
    """
...
if __name__ == "__main__":
    # Test the analyzer
    analyzer = BabblIntelligenceAnalyzer()
    
    # Load transcript from file (created by transcript_extractor.py)
    try:
        with open('extracted_transcript.json', 'r') as f:
        ...
        results = analyzer.comprehensive_analysis(transcript_text)
        
        print(f"\n⏱️  Analysis completed in {results.processing_time:.2f} seconds")
        print(f"\n📊 BUSINESS INSIGHTS:\n{results.business_insights}")
        print(f"\n🏢 ENTITY ANALYSIS:\n{results.entity_analysis}")
        print(f"\n⚔️  COMPETITIVE INTELLIGENCE:\n{results.competitive_intelligence}")
        print(f"\n💰 FINANCIAL IMPLICATIONS:\n{results.financial_implications}")
        
        # Test querying
        print(f"\n❓ TESTING CONTENT QUERIES:")
        questions = [
            "What companies are mentioned in this video?",
            "What is the overall sentiment toward Apple?",
            "Are there any financial risks mentioned?"
        ]
        
        for question in questions:
            answer = analyzer.query_content(transcript_text, question)
            print(f"\nQ: {question}")
            print(f"A: {answer}")
        
    except FileNotFoundError:
        print("❌ No transcript file found. Run transcript_extractor.py first!")
    except Exception as e:
        print(f"❌ Error: {e}")